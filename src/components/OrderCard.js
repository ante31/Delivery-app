...
import { formatTimeToEuropean } from '../services/formatTimeToEuropean';

const OrderCard = ({ order, index, openIndexes, toggleCollapse }) => {
  ...
  return (
    <Card key={order.id} className="mb-3 shadow-sm" style={{ borderRadius: '12px' }}>
      <Card.Body>
        ...
        {/* vrijeme */}
        <Card.Text>
          {formatTimeToEuropean(order.time)} - {formatTimeToEuropean(order.deadline)}
        </Card.Text>
      </Card.Body>
    </Card>
  )
